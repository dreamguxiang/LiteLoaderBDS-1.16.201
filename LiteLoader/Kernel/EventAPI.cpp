...
#include <MC/ItemUseInventoryTransaction.hpp>
 TInstanceHook(char, "?checkBlockPermissions@BlockSource@@QEAA_NAEAVActor@@AEBVBlockPos@@EAEBVItemStackBase@@_N@Z",
               BlockSource,Player* entity,BlockPos* blockPos,unsigned __int8 face,ItemStackBase* item,bool generateParticle) {
     if (entity->isPlayer()) {
         IF_LISTENED(PlayerPlaceBlockEvent) {
            auto pl = (Player*)entity;
             ...
             if (!ev.call()) { // this pointer is not used.
                ((ItemUseInventoryTransaction*)nullptr)->resendBlocksAroundArea(*pl, *blockPos, face);
                 return false;
             }
         }
         IF_LISTENED_END(PlayerPlaceBlockEvent)
     }
     return original(this, entity, blockPos, face, item, generateParticle);
 }
...
/////////////////// HopperSearchItem ///////////////////
TInstanceHook(bool, "?_tryPullInItemsFromAboveContainer@Hopper@@IEAA_NAEAVBlockSource@@AEAVContainer@@AEBVVec3@@@Z",
                       Hopper,BlockSource* bs, Container* container, Vec3* pos) {
     bool isMinecart = this->mIsEntity; // IDA Hopper::Hopper 未知正确
