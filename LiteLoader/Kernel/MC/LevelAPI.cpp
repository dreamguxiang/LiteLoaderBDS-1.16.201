...
std::unique_ptr<CompoundTag> getPlayerOriginTag(Player& player) {
 ...
 bool Level::executeCommand(const string& cmd) {
    ...
    std::unique_ptr<ServerCommandOrigin> origin(new ServerCommandOrigin(requestID, *Global<ServerLevel>, CommandPermissionLevel(4),Global<Level>->getDimension(0)));
...
ItemStack* Level::getItemStackFromId(short itemId, int aux) {
    ...
    if (item)
        return new ItemStack(item.get()->getFullItemName(), 1, aux,0);
    return nullptr;
}
