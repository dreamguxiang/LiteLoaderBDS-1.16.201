...
Local<Value> convertResult(DynamicCommand::Result const& result) {
    ...
    switch (result.type) {
        ...
        case DynamicCommand::ParameterType::Item: {
            auto itemname = result.getRaw<CommandItem>().createInstance(1, 1, nullptr, true).value_or(ItemInstance::EMPTY_ITEM).getItem()->getFullItemName();
            return ItemClass::newItem(new ItemStack(itemname, 1, 1, 0));
        }
